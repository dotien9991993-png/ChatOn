...
export default function RegisterPage() {
  ...
  async function handleGoogleLogin() {
    try {
      ...
      if (err.message?.includes('provider is not enabled') || err.message?.includes('Provider')) {
        toast.error('Đăng nhập Google chưa được kích hoạt. Vui lòng dùng email.');
      } else {
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-10 sm:px-10">
        <div className="w-full max-w-md">
          ...
          >
            ...
            <span className="text-sm font-medium text-slate-700">Tiếp tục với Google</span>
          </button>
